...
static IMFTopology* BuildPartialTopology(MediaSource* mediaSource, MediaSink* mediaSink, WORD streamNumber, IMFTransform* __audioEncoder, AudioEncoderParameters* encoderParameters)
{
...
//  IMFTransform* audioEncoder = nullptr;
  ...
  do
  {
    if (!SUCCEEDED(hr = MFCreateTopology(&mfTopology)))
      break;

    sourceNode = mediaSource->CreateTopologySourceNode();
...
// moved to caller    audioEncoder = mediaSink->GetAudioEncoderForStream(1);
    ...
    outputNode = mediaSink->CreateTopologyOutputNode(streamNumber);
  ...
  if (FAILED(hr))
  {
    throw std::exception("Unable to construct encoding topology");
  }
...
static void RunMediaSession(IMFMediaSession* mfMediaSession)
{
  ...
  */
  ...
  while (1)
  {
    ...
    case MEEndOfPresentation:
    {
      ...
      {
        hr = UpdateVbrStreamProperties(mfMediaSession, mediaSink);
        if (FAILED(hr))
        {
          goto done;
        }
...
// TODO      hr = mfMediaSession->Shutdown();
      goto done;
    }
    if (FAILED(hr))
    {
...
void AudioEncoder::Encode(MediaSource* mediaSource, MediaSink* mediaSink, WORD streamNumber, AudioEncoderParameters* encoderParameters)
{
  IMFTopology* mfTopology = nullptr;
  IMFTransform* audioEncoder = nullptr;
  IMFMediaSession* mfMediaSession = nullptr;
  ...
  do
  {
    audioEncoder = mediaSink->GetAudioEncoderForStream(1);

    mfTopology = BuildPartialTopology(mediaSource, mediaSink, streamNumber, audioEncoder, encoderParameters);

    if (!SUCCEEDED(hr = MFCreateMediaSession(nullptr, &mfMediaSession)))
      break;

    if (!SUCCEEDED(hr = mfMediaSession->SetTopology(MFSESSION_SETTOPOLOGY_IMMEDIATE, mfTopology)))
      break;

    RunMediaSession(mfMediaSession);

    // if VBR, update sink streams with properties computed during the encoding process
  ...
  if (mfTopology) mfTopology->Release();

  if (FAILED(hr))
    throw std::exception("Unable to encode file");
